...
# Inicialização do Conector na Sessão do Streamlit
if "connector" not in st.session_state:
    st.session_state.connector = DigitalTwinConnector()
    st.session_state.loop = asyncio.new_event_loop()
    asyncio.set_event_loop(st.session_state.loop)
    try:
        st.session_state.loop.run_until_complete(st.session_state.connector.connect_and_subscribe())
    except Exception as e:
        st.error(f"Erro ao conectar ao CODESYS OPC UA: {e}")

dt: DigitalTwinConnector = st.session_state.connector
...
with st.sidebar:
    ...
    with col_c1:
        if st.button("▶️ START", use_container_width=True, type="primary"):
            loop.run_until_complete(dt.start_plant())
            st.toast("Comando START enviado para o CLP!", icon="🚀")
            st.rerun()

    with col_c2:
        if st.button("🔄 RESET", use_container_width=True):
            loop.run_until_complete(dt.reset_plant())
            st.toast("Comando RESET enviado! Falhas limpas.", icon="🔄")
            st.rerun()

    if st.button("🛑 PARADA DE EMERGÊNCIA", use_container_width=True):
        loop.run_until_complete(dt.emergency_stop(reason="Parada de Emergência acionada pelo operador no Dashboard"))
...
with col4:
    ...
    <div class="metric-card">
        ...
        <div style="color: #f59e0b; font-size: 1.4rem; font-weight: bold;">{", ".join(petri['active_places']) if petri['active_places'] else 'p1'}</div>
    </div>
    """, unsafe_allow_html=True)


# =============================================================================
# ABAS PRINCIPAIS: VISÃO SINÓTICA, REDE DE PETRI, AUDITORIA, MODELO AAS
# =============================================================================
st.markdown("<br>", unsafe_allow_html=True)
tab1, tab2, tab3, tab4 = st.tabs([
    "🖥️ Chão de Fábrica (Sinótico 2D)",
    "🕸️ Rede de Petri ao Vivo",
...
with tab2:
    ...
    active_places = petri["active_places"]

    # Criação do Grafo Visual usando Plotly
    places_info = {
...
with tab3:
    ...
    if recent_events:
        ...
        df_display = df_events[["timestamp_iso", "tag_name", "value", "quality", "source", "is_valid"]]
        df_display.columns = ["Timestamp (UTC)", "Variável", "Valor Sanitizado", "Qualidade", "Origem", "Válido"]
...
with tab4:
    ...
    # Botão para download do arquivo AAS JSON
...
# -----------------------------------------------------------------------------
# AUTO-REFRESH AUTOMÁTICO DO DASHBOARD (A CADA 1.5 SEGUNDOS)
# -----------------------------------------------------------------------------
time.sleep(1.0)
st.rerun()
